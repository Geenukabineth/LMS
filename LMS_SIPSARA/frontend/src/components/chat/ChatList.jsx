...
const ChatList = ({ chats, selectedChat, onSelectChat, onlineUsers, onChatDeleted, onNotification }) => {
  ...
  const handleDeleteChat = async (chatId) => {
    try {
      // Delete the chat using the chat detail endpoint
      await apiCall(API_ENDPOINTS.CHAT_DETAIL(chatId), {
        method: 'DELETE',
      });
          ...
          >
            <div className="flex items-center flex-1 min-w-0 space-x-3">
              <div className="relative flex-shrink-0">
                ...
                {chat.user && onlineUsers.has(chat.user?.id) && (
            ...
            <div className="p-2 mb-2 border border-red-200 rounded-lg bg-red-50">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 px-2 py-1 text-sm text-white bg-red-500 rounded hover:bg-red-600"
                >
                  Delete
                </button>
                <button
                  onClick={() => setDeleteConfirm(null)}
                  className="flex-1 px-2 py-1 text-sm text-gray-700 bg-gray-300 rounded hover:bg-gray-400"
                >
                  Cancel
                </button>
              </div>
