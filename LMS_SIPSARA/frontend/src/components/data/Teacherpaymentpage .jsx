...
const TeacherPaymentPage = () => {
  ...
  const fetchPayments = async () => {
    try {
      ...
      const formattedData = Array.isArray(data) ? data.map(item => ({
        ...
        // ✅ FIX 1: Normalize status to lowercase to prevent mismatch
        status: (item.status || 'successful').toLowerCase(), 
      })) : [];
  ...
  const filteredPayments = payments
    .filter((payment) => {
      // ✅ FIX 2: Case-insensitive comparison for filtering
      if (filterStatus !== 'all' && payment.status !== filterStatus.toLowerCase()) {
        return false;
      }
  ...
  const getStatusBadge = (status) => {
    ...
    return (
      <span className={`px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide ${statusColors[normalizedStatus] || 'bg-gray-100 text-gray-800'}`}>
        {statusLabel}
      </span>
    );
  };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-4 border-gray-200 rounded-full border-t-orange-500 animate-spin"></div>
    ...
    <div className="min-h-screen px-4 py-8 bg-gray-50">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col gap-4 p-6 mb-6 text-white rounded-lg shadow-md bg-gradient-to-r from-orange-600 to-red-500 md:flex-row md:justify-between md:items-center">
          <h1 className="mb-2 text-2xl font-bold text-white uppercase">Payment History</h1>
        </div>
        ...
        <div className="flex flex-col items-stretch gap-4 p-4 mb-6 bg-white rounded-lg shadow md:flex-row md:items-center">
          <div className="flex-1 min-w-0">
            <input
              ...
              className="w-full px-4 py-2 text-sm transition border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
            />
          </div>
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <select
              ...
              className="px-4 py-2 text-sm transition bg-white border border-gray-300 rounded-lg cursor-pointer focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent"
            >
              <option value="all">All Status</option>
              {/* ✅ FIX 5: Values are lowercase to match the normalized data */}
            ...
            <button
              ...
              className="px-4 py-2 text-sm font-medium text-white transition bg-orange-500 rounded-lg hover:bg-orange-600 whitespace-nowrap"
            >
              ↻ Refresh
            </button>
          </div>
